...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--train-before", default=os.environ.get("PTF_TRAIN_BEFORE", "2026-04-01"))
    ...
    p.add_argument(
        ...
        type=float,
        default=0.15,
        help="Eğitim setinin early stopping için ayrılacak validation oranı (varsayılan 0.15)",
    )
    return p.parse_args()
...
def main() -> int:
    ...
    if args.log_target:
        ...
        Y_test_eval = np.log1p(Y_test.clip(lower=0))
        print("Log-transform aktif: fiyat log(price+1) ile egitiliyor.")
    else:
        Y_train_fit = Y_train
        Y_test_eval = Y_test
    ...
    Y_val = Y_train_fit.iloc[-val_size:]
    X_tr  = X_train.iloc[:-val_size]
    Y_tr  = Y_train_fit.iloc[:-val_size]
    ...
    mape_safe = np.mean(np.abs((Y_test_orig - y_pred) / np.clip(np.abs(Y_test_orig), 100, None))) * 100
    print("TEST MAE:", mae)
    print("TEST RMSE:", rmse)
    print(f"TEST MAPE (min_denom=100 TL): {mape_safe:.2f}%")
    ...
    df_all_out = out_path.parent / (out_path.stem + "_df_all.xlsx")
    df_all_cols_out = ["date", "price"] + [c for c in df_all.columns if c not in ["date", "price"]]
    df_all_cols_out = [c for c in df_all_cols_out if c in df_all.columns]
    df_all[df_all_cols_out].to_excel(df_all_out, index=False)
    print("df_all (tum satirlar) kaydedildi:", df_all_out)
