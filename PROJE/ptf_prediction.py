...
def load_env_optional() -> None:
    """Proje kökündeki .env dosyasını yükle (python-dotenv varsa)."""
...
def load_credentials() -> tuple[str, str]:
    ...
    if not user or not pwd:
        raise RuntimeError(
            "EPIAS kimlik bilgisi eksik. Ortam degiskenlerini ayarlayin:\n"
            "  EPIAS_USERNAME\n"
            "  EPIAS_PASSWORD\n"
            "Veya proje kokunde .env dosyasi olusturun (.env.example'a bakin)."
        )
    return user, pwd
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--start-date",
        default=os.environ.get("PTF_START_DATE", "2025-01-01"),
        help="YYYY-MM-DD (varsayilan: PTF_START_DATE veya 2025-01-01)",
    )
    p.add_argument(
        "--end-date",
        default=os.environ.get("PTF_END_DATE", "2025-12-24"),
        help="YYYY-MM-DD",
    )
    p.add_argument(
        "--train-before",
        default=os.environ.get("PTF_TRAIN_BEFORE", "2025-10-01"),
        help="Bu tarihten once egitim (test bu tarih ve sonrasi)",
    )
    p.add_argument(
        "--forecast-input",
        type=Path,
        default=Path(os.environ.get("PTF_FORECAST_INPUT", str(default_forecast))),
        help="Gelecek tahmin girdisi Excel yolu",
    )
    p.add_argument(
        "--output",
        type=Path,
        default=Path(os.environ.get("PTF_FORECAST_OUTPUT", str(default_out))),
        help="Cikti Excel yolu",
    )
    p.add_argument(
        "--lgbm-device",
        choices=["cpu", "gpu", "auto"],
        default=os.environ.get("LGBM_DEVICE", "cpu"),
        help="LightGBM cihazi: auto=GPU dene, basarisizsa CPU (varsayilan: cpu)",
    )
    p.add_argument(
        "--no-plots",
        action="store_true",
        help="Matplotlib grafiklerini gosterme",
    )
    p.add_argument(
        "--no-meteo",
        action="store_true",
        help="Open-Meteo hava verisini cekme / birlestirme",
    )
    p.add_argument(
        "--meteo-lat",
        type=float,
        default=float(os.environ.get("METEO_LAT", "41.0082")),
        help="Open-Meteo enlem (varsayilan: Istanbul)",
    )
    p.add_argument(
        "--meteo-lon",
        type=float,
        default=float(os.environ.get("METEO_LON", "28.9784")),
        help="Open-Meteo boylam (varsayilan: Istanbul)",
    )
    p.add_argument(
        "--debug-save",
        action="store_true",
        help="df_merged, df_future, df_all, forecast_df dosyalarini masaustune Excel olarak kaydet",
    )
    p.add_argument(
        "--debug-dir",
        type=Path,
        default=None,
        help="Debug cikti klasoru (yoksa masaustu/PTF_DEBUG_<zaman>)",
    )
    return p.parse_args()
    ...
    while part_start <= end:
        ...
        if r.status_code == 200:
            ...
            if items:
                ...
                print(f"{label}: {part_start.date()} - {part_end.date()} arasi {len(items)} kayit cekildi.")
            else:
                print(f"{label}: {part_start.date()} - {part_end.date()} arasi veri yok.")
        else:
...
def fetch_kgup(start_date: str, end_date: str, tgt: str) -> pd.DataFrame:
    ...
    while part_start <= end:
        ...
        if r.status_code == 200:
            ...
            if items:
                all_data.extend(items)
                print(f"KGUP: {part_start.date()} - {part_end.date()} arasi {len(items)} kayit cekildi.")
            else:
                print(f"KGUP: {part_start.date()} - {part_end.date()} arasi veri yok.")
        else:
...
def validate_merge_frames(df_ptf: pd.DataFrame, df_consumption: pd.DataFrame, df_kgup: pd.DataFrame) -> None:
    """Birleştirme öncesi şema kontrolü."""
    if df_ptf.empty:
        raise ValueError("PTF verisi boş; tarih aralığını ve API yanıtını kontrol edin.")
    if "date" not in df_ptf.columns:
        raise ValueError(f"PTF yanıtında 'date' kolonu yok. Kolonlar: {list(df_ptf.columns)}")
    if not df_consumption.empty and "date" not in df_consumption.columns:
        raise ValueError(f"Tüketim yanıtında 'date' kolonu yok. Kolonlar: {list(df_consumption.columns)}")
    if not df_kgup.empty and "date" not in df_kgup.columns:
        raise ValueError(f"KGÜP yanıtında 'date' kolonu yok. Kolonlar: {list(df_kgup.columns)}")
...
def normalize_consumption_column(df: pd.DataFrame) -> pd.DataFrame:
    """
    Farkli endpointlerden gelen tuketim kolonlarini tek isimde toplar.
    load-estimation-plan endpoint'i genelde 'lep' dondurur.
    """
    df = df.copy()
...
def build_lag_map(_df: pd.DataFrame) -> dict:
    """Sadece EPİAŞ/üretim lagları; meteoroloji kolonları için lag yok."""
    return {**BASE_LAG_MAP}
...
def align_hourly_stamp(series: pd.Series) -> pd.Series:
    """EPİAŞ ile Open-Meteo saat damgalarini ayni (yerel, saat basi) anahtarda birlestirir."""
...
def fetch_meteo_forecast_short(filter_end_date: str, latitude: float, longitude: float) -> pd.DataFrame:
    """Bugun + day+1 (forecast_days=2); kullanici kodundaki filtre ile."""
    r = requests.get(
        OPEN_METEO_FORECAST,
        params={
            "latitude": latitude,
...
def fetch_meteo_forecast_range(start_date: str, end_date: str, latitude: float, longitude: float) -> pd.DataFrame:
    """Masaustu forecast Excel araligi icin (Open-Meteo start_date/end_date)."""
    r = requests.get(
        OPEN_METEO_FORECAST,
        params={
            "latitude": latitude,
...
def meteo_raw_to_merge_df(df: pd.DataFrame) -> pd.DataFrame:
    """time + hourly -> date + turkce sutun adlari."""
...
def get_meteo_for_epias_merged(
    epias_start: str,
    epias_end: str,
    latitude: float,
    longitude: float,
    ...
    """
    Archive: EPİAŞ araligi ile kesisim, dune kadar.
    Uzerine forecast_days=2 (yarina kadar filtre) — kullanici akisi.
    """
    tz = ZoneInfo(METEO_TIMEZONE)
    bitis = (datetime.now(tz).date() - timedelta(days=1)).strftime("%Y-%m-%d")
    yarın = (datetime.now(tz).date() + timedelta(days=1)).strftime("%Y-%m-%d")
    ...
    print(f"Hava forecast (2 gun, <= {yarın}): birlestiriliyor")
    parts.append(fetch_meteo_forecast_short(yarın, latitude, longitude))
    ...
    print(f"Hava toplam (merge oncesi): {len(out)} saat | {out['date'].min()} -> {out['date'].max()}")
    return out


def get_meteo_for_forecast_excel(
    d_min: pd.Timestamp,
    d_max: pd.Timestamp,
    latitude: float,
    longitude: float,
    ...
    """
    Masaustu forecast satirlari: min-max tarih araliginda saatlik hava.
    Aralik uzunsa Open-Meteo limitine (tipik ~16 gun) takilabilir; o zaman araligi bol.
    """
    ...
    if start_d <= yday:
        ...
        if start_d <= h_end:
            print(f"Hava (Excel archive parcasi): {start_d} -> {h_end}")
    ...
    if f0 <= end_d:
        print(f"Hava (Excel forecast araligi): {f0} -> {end_d}")
    ...
    print(f"Hava Excel eslemesi: {len(out)} saat | {out['date'].min()} -> {out['date'].max()}")
    return out


def merge_meteo_hourly(left: pd.DataFrame, meteo: pd.DataFrame) -> pd.DataFrame:
    """`date` uzerinden (saat hizali) hava kolonlarini birlestirir."""
    left = left.copy()
    ...
    if meteo is None or meteo.empty:
        print("Uyari: hava verisi bos; meteo sutunlari eklenmedi.")
        return left
    m = meteo.copy()
    m["date"] = align_hourly_stamp(m["date"])
    m = m.drop_duplicates(subset=["date"], keep="first")
    out = left.merge(m, on="date", how="left")
    return out
...
def resolve_lgbm_device(device: str) -> str:
    """GPU isteniyorsa dene; LightGBM/OpenCL yoksa CPU'ya düş."""
    if device == "cpu":
        return "cpu"
    if device == "gpu":
        # LightGBM: device='gpu' CUDA build gerektirir; yoksa hata verir — yakala ve CPU'ya geç
        try:
...
def main() -> int:
    ...
    # load-estimation-plan ile gelen 'lep', 'consumption' ile ayni bilgiyi tasir.
    # Modelin future tarafta da ayni sutun setiyle calismasi icin lep'i disari aliyoruz.
    ...
    if not args.no_meteo:
        meteo_hist = get_meteo_for_epias_merged(
            args.start_date,
            args.end_date,
            args.meteo_lat,
            args.meteo_lon,
    ...
    if train_df.empty:
        raise ValueError("Eğitim kümesi boş: --train-before ve tarih aralığını kontrol edin.")
    if test_df.empty:
        print("Uyarı: test kümesi boş; metrikler anlamsız olabilir.")
    ...
    if not args.no_plots:
        daily = (
            df_test.set_index("date")[["actual", "pred"]].resample("D").mean()
        )
    ...
    if not forecast_path.is_file():
        raise FileNotFoundError(
            f"Tahmin girdisi bulunamadı: {forecast_path}\n"
            "--forecast-input ile yolu verin veya masaüstüne forecast.xlsx koyun."
        )
    ...
    rename_map = {
        ...
        "Tüketim Miktar(MWh)": "consumption",
        "Tuketim Miktari(MWh)": "consumption",
        "Tuketim Miktar(MWh)": "consumption",
    }
    ...
    if not args.no_meteo:
        ...
        if pd.notna(d_min) and pd.notna(d_max):
            meteo_f = get_meteo_for_forecast_excel(
                d_min,
                d_max,
                args.meteo_lat,
                args.meteo_lon,
    ...
    holiday_dates_set = set(tr_h.keys())
    df_all["resmi_tatil"] = dt.dt.date.isin(holiday_dates_set).astype(int)
    ...
    if args.debug_save:
        ...
        print("Debug DataFrame dosyalari:", dbg)
